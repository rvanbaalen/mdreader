...
struct MDReaderEntry {
    // appVersion is defined in Version.swift, generated by build scripts
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var currentVersion: String { appVersion }
        ...
        NSWorkspace.shared.setDefaultApplication(at: appURL, toOpen: mdType) { error in
            DispatchQueue.main.async {
                if let error {
                    ...
                    wc.webView.evaluateJavaScript("app.showToast('success', 'mdreader is now your default markdown reader')")
                }
            }
        }
    }
    ...
    @objc func menuCopyLogs() {
        ...
        if let wc = keyWindowController(), wc.webReady {
            wc.webView.evaluateJavaScript("app.showToast('success', 'Logs copied to clipboard')")
        }
    }
    ...
    @objc func showAbout() {
        ...
        guard let wc = keyWindowController() ?? windowControllers.first else { return }
        wc.webView.evaluateJavaScript("app.showAbout('\(v)', '\(c)', '\(b)')")
    }
}
